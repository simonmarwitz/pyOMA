...
class PreProcessApp(QtWidgets.QMainWindow, design.Ui_MainWindow):
    def __init__(self, parent=None):
        ...
        self.plotLogState = 0
        self.visualizationTypeSelect.currentTextChanged.connect(
            self.updatePlot)
        ...
        self.toggle_selectorFreq = RectangleSelector(self.canvasFreq.axes, self.canvasFreqSelection,
                                                     ...
                                                     button=[1],
                                                     interactive=False)
        self.canvasTime.mpl_connect(
            'button_press_event',
            self.canvasTimeZoomOut)
        self.canvasFreq.mpl_connect(
            'button_press_event',
            self.canvasFreqZoomOut)
        ...
        self.filterUI.filterButtonCancel.clicked.connect(
            self.filterButtonCancelClick)
        self.filterUI.filterButtonOK.clicked.connect(self.filterButtonOKClick)
        self.filterUI.filterType.currentIndexChanged.connect(
            self.filterChagesFunc)
        self.filterUI.filterDesign.currentIndexChanged.connect(
            self.filterChagesFunc)
    ...
    def filterButtonOKClick(self):
        ...
        try:
            ...
            QtWidgets.QMessageBox.critical(
                self, "Error", "Inputs can only be numbers.")
    ...
    def openTestFileClick(self):
        ...
        self.startTimeEdit.setText("0")
        self.endTimeEdit.setText(
            str((self.data.total_time_steps) / self.sampling_rate))
        self.startFreqEdit.setText("0")
    ...
    def updatePlot(self):
        ...
        if self.visualizationTypeSelect.currentText() == "Time, FFT":
            # get data to plot
            time, accel = self.data.get_time_accel(
                int(self.channelBox.currentText()))
            ...
            freq_a = int(4 * (float(self.startFreqEdit.text()) /
                              self.sampling_rate / 2) * len(fft_freq))
            freq_z = int(4 * (float(self.endFreqEdit.text()) /
                              self.sampling_rate / 2) * len(fft_freq))
            ...
            # not functional
            psd_vector = np.abs(
                psd_mats[int(self.channelBox.currentText()), :])
            psd_vector = np.mean(psd_vector, axis=0)

            freq_a = int(4 * (float(self.startFreqEdit.text()) /
                              self.sampling_rate / 2) * len(psd_vector))
            freq_z = int(4 * (float(self.endFreqEdit.text()) /
                              self.sampling_rate / 2) * len(psd_vector))
    ...
    def nextChanButtonClick(self):
        items = [self.channelBox.itemText(i)
                 for i in range(self.channelBox.count())]
        items = len(items) - 1
    ...
    def buttonSaveFigureClick(self):
        fileName, _ = QtWidgets.QFileDialog.getSaveFileName(
            self, "Save time signal figure")
        fig = self.canvasTime.figure
        fig.savefig(fileName, dpi=300)
        fileName, _ = QtWidgets.QFileDialog.getSaveFileName(
            self, "Save frequency sprectrum figure")
...
#        self.canvasFreq.axes.relim()
    ...
    def canvasTimeZoomOut(self, event):
        if event.button == 3:
            ...
            self.endTimeEdit.setText(
                str(round((self.data.total_time_steps - 1) / self.sampling_rate, 2)))
