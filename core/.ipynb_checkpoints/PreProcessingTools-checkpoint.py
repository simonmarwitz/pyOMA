    ...
    def plot_signals(self,
        ...
        for axt, axf, channel in zip(axest, axesf, channel_numbers):
            ...
            if timescale == 'lags':
                ...
                prep_data.plot_correlation(prep_data.n_lags, [channel], axt, timescale, refs,
                                      plot_kwarg_dict.copy(),
                                      refs_only=refs_only, method=method)
            else:
                prep_data.plot_timeseries(channels=[channel], ax=axt, scale='timescale', **plot_kwarg_dict.copy())
                
            axt.grid(True, axis='y', ls='dotted')
            
            #omitting **kwargs here to not trigger recomputation, except refs_only
            prep_data.plot_psd(n_lines, [channel], axf, psd_scale, refs,
                          plot_kwarg_dict.copy(),
                          refs_only=refs_only, method=method)
