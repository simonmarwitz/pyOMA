...
class SSIData(ModalBase):
    ...
    def compute_modal_params(self, max_model_order=None, multiprocessing=True):
        ...
        if multiprocessing:
            ...
            # balanced allocation of work (numpy.linalg.eig takes approx. n^3
            # operations)
            work_slice_size = sum(
                [n**3 for n in range(max_model_order)]) / max_model_order
            current_size = 0
            current_orders = []
            for order in range(0, max_model_order, 1):
                ...
                if current_size >= work_slice_size:
                    pool.apply_async(
                        self.multiprocess_evd, args=(
                            A_full, current_orders, return_dict))
                    current_orders = []
                    current_size = 0
            pool.close()
            pool.join()
        ...
        for order in range(0, max_model_order, 1):
            ...
            if multiprocessing:
                ...
                eigenvectors_single, eigenvalues_single = self.remove_conjugates_new(
                    eigenvectors_paired, eigenvalues_paired)
...
#         self.state[1] = False # previous state matrices are invalid now
    ...
    def compute_state_matrices(self, max_model_order=None):
        ...
        if use_q:
            ...
            # it was tried with different data sets and equal results were
            # obtained regarding the stable modes
            Q_1_T = self.Q_1_T
    ...
    def compute_modal_params(self, plot_=False):
        ...
        for order in range(1, max_model_order):
            ...
            if estimation_algo == 0:
                ...
                On_up = O[:num_analised_channels * num_block_rows, :order]
                On_upi = np.linalg.pinv(On_up)
                QSR1 = np.vstack([np.hstack([On_upi.dot(R_41),
                                             On_upi.dot(R_42),
                                             np.zeros((order,
                                                       num_analised_channels - num_ref_channels))]),
                                  np.hstack([R_21,
                                             R_22,
                                             np.zeros((num_ref_channels,
                                                       num_analised_channels - num_ref_channels))]),
                                  np.hstack([R_31,
                                             R_32,
                                             R_33])])

                VVT = np.identity(num_ref_channels *
                                  num_block_rows) - V.dot(V.T)

                QSR2 = np.vstack([np.hstack([VVT,
                                             np.zeros((num_ref_channels * num_block_rows,
                                                       num_analised_channels))]),
                                  np.hstack([np.zeros((num_analised_channels,
                                                       num_ref_channels * num_block_rows)),
                                             np.identity(num_analised_channels)]),
                                  ])
                ...
                AC = np.vstack([On_upi.dot(R_41), R_21, R_31]).dot(
                    V.dot(S_2[:order, :order]))
                ...
                Y_i_i = np.vstack([np.hstack([R_21,
                                              R_22,
                                              np.zeros((num_ref_channels,
                                                        num_analised_channels - num_ref_channels))]),
                                   np.hstack([R_31,
                                              R_32,
                                              R_33])]).dot(np.vstack((Q_1_T,
                                                                      Q_2_T,
                                                                      Q_3_T)))
            ...
            try:
                ...
                print(
                    'Correlations of residuals are not symmetric. Skiping Modal Contributions')
                continue
#                 try:
#                     Q = (Q + Q.T)*0.5
#                     R = (R + R.T)*0.5
...
#                     continue
            ...
            if plot_ and order > 10:
                print(modal_contributions[order, :][modal_contributions[order, :] != 0], np.sum(
                    modal_contributions[order, :]))
...
#                 plot.show()
                ...
                fig, axes = plot.subplots(
                    len(conj_indices) + 1, 2, sharex='col', sharey='col', squeeze=False)
                # print(axes)
                j4 = int(np.floor(j / 4))
                ft_freq = np.fft.rfftfreq(
                    j4, d=(1 / self.prep_data.sampling_rate))
                ...
                for (ax1, ax2), meas_synth in zip(axes, meas_synth_single):
                    ...
                    for i in range(1, 4):
                        ft += np.fft.rfft(meas_synth[0,
                                                     i * j4:(i + 1) * j4] * np.hanning(j4))
                    ft /= 4
                ...
                for i in range(1, 4):
                    ft_synth += np.fft.rfft(
                        meas_synth_all[0, i * j4:(i + 1) * j4] * np.hanning(j4))
                ft_synth /= 4
