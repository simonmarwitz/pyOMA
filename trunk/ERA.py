...
class ERA(object):
    ...
    def CalculateFRF(self):
        ...
        acceleration_fft = np.zeros((num_time_steps//2+1, num_channels), dtype=complex)

        F_fft=np.fft.rfft(np.hamming(num_time_steps)*self.prep_data.F)
        
        for channel in range(num_channels):# loop over channels
            fft_this_channel = np.fft.rfft(np.hamming(num_time_steps)*measurement[:,channel])
    ...
    def build_hankel_matrix(self, num_block_columns):
        ...
        Hankel_matrix=np.zeros((num_channels*num_block_rows,num_block_columns), dtype=complex)
        for i in range(0,num_block_rows):
            j=i+1
            this_block = IRFT[0:num_channels, j:(num_block_columns+j)]
            begin_row = i*num_channels
            Hankel_matrix[begin_row:(begin_row+num_channels),0:num_block_columns] = this_block
