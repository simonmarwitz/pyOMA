...
def main():
    import os
    os.chdir('/ismhome/staff/womo1998/Projects/2018_eigensystem_realization_algorithm/code/')
    ...
    #acceleration
    measurement = data['X_dotdot']
    #plot.plot(measurement.T)
    #plot.show()
    #added by anil
    ImpulseList= np.fft.rfft(measurement, axis=1)#
    #ImpulseList = DFT(measurement)
    ImpulseList= np.asarray(ImpulseList)

    #print(measurement.shape)
    measurement=ImpulseList.T
    #print(measurement.shape)
    ...
    '''
    Your Task later:
    Add the possibility to add Input Data (F) here, without interfering with the current behaviour
    '''
    
    #add Channel-DOF assignments: [ (chan_num, node_name, az, elev, chan_name), ...]
    ...
    '''
    Your Task:
    This class currently is the covariance driven SSI method.
    Replace it with the ERA algorithm, but maintain the structure of the 
    class variables, s.t. all pre- and post processing routines will still work
    '''
